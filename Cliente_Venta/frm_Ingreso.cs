...
namespace Cliente_Venta
{
    public partial class frm_Ingreso : Form
    {
        ...
        private static frm_Ingreso _instancia;
        ServiceIngreso.Service_IngresoClient sc = new ServiceIngreso.Service_IngresoClient();

        public static frm_Ingreso GetInstancia()
        {
            if (_instancia == null)
            {
                _instancia = new frm_Ingreso();
            }
            return _instancia;
        }
        ...
        {
            this.dataListadoDetalle.DataSource = sc.MostrarDetalle(this.txtIdingreso.Text);
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = sc.Anular(Convert.ToInt32(Codigo));
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (this.IsNuevo)
                    {
                        ...
                        {
                            rpta = sc.Insertar(Idtrabajador, Convert.ToInt32(this.txtIdproveedor.Text),
                           dtFecha.Value, cbTipo_Comprobante.Text, txtSerie.Text, txtCorrelativo.Text,
                           Convert.ToDecimal(txt_igv.Text), "EMITIDO", dtDetalle);
                        }
                        else
                        {
                           rpta = sc.Insertar(Idtrabajador, Convert.ToInt32(this.txtIdproveedor.Text),
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (Convert.ToDecimal(txtPrecio_Venta.Text) < Convert.ToDecimal(txtPrecio_Compra.Text))
                    {
                        this.MensajeError("El precio de venta no puede ser menor al precio de compra.");
                        return;
                    }
        ...
        private void dataListado_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(this.dataListado.CurrentRow.Cells["tipo_comprobante"].Value) == "FACTURA")
            {
